<template>
  ...
  >
    ...
    <template v-slot:top-right>
      <q-input dense debounce="300" v-model="filter" placeholder="Buscar" :disable="noHayDatos">
    ...
    <template v-for="(_, slot) in $slots" v-slot:[slot]="scope">
      <slot v-if="slot !== 'body' && !String(slot).startsWith('body-cell-')" :name="slot" v-bind="scope" />
    </template>
...
const customFilter = (rows: any[], terms: string, cols: any[], getCellValue: (col: any, row: any) => any) => {
  const lowerTerms = terms ? terms.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase() : '';
  return rows.filter(row => {
    return cols.some(col => {
      const val = getCellValue(col, row);
      const strVal = (val === null || val === undefined) ? '' : String(val);
      return strVal.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().includes(lowerTerms);
    });
  });
};
...
const exportTable = () => {
  ...
  const content = [visibleColumns.value.map((col: any) => wrapCsvValue(col.label))].concat(
    tableRef.value.computedRows.map((row: any) => visibleColumns.value.map((col: any) => wrapCsvValue(
